...
public final class MAC
{
    ...
    public static final void checkMacList(final String[] macs) {
        for (int i = 0; i < macs.length; ++i) {
            getKeyLen(macs[i]);
        }
    }
    
    public static final int getKeyLen(final String type) {
    ...
    public MAC(final String type, final byte[] key) {
        if (type.equals("hmac-sha1")) {
    ...
    public final void update(final byte[] packetdata, final int off, final int len) {
        this.mac.update(packetdata, off, len);
    }
    
    public final void getMac(final byte[] out, final int off) {
