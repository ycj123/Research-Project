...
public class DES implements BlockCipher
{
    ...
    static {
        DES.Df_Key = new short[] { 1, 35, 69, 103, 137, 171, 205, 239, 254, 220, 186, 152, 118, 84, 50, 16, 137, 171, 205, 239, 1, 35, 69, 103 };
        DES.bytebit = new short[] { 128, 64, 32, 16, 8, 4, 2, 1 };
        ...
        DES.totrot = new byte[] { 1, 2, 4, 6, 8, 10, 12, 14, 15, 17, 19, 21, 23, 25, 27, 28 };
    ...
    protected int[] generateWorkingKey(final boolean encrypting, final byte[] key, final int off) {
        final int[] newKey = new int[32];
        final boolean[] pc1m = new boolean[56];
        final boolean[] pcr = new boolean[56];
        for (int j = 0; j < 56; ++j) {
            final int l = DES.pc1[j];
            pc1m[j] = ((key[off + (l >>> 3)] & DES.bytebit[l & 0x7]) != 0x0);
        }
        for (int i = 0; i < 16; ++i) {
            int m;
            if (encrypting) {
            ...
            for (int k = 0; k < 24; ++k) {
                if (pcr[DES.pc2[k]]) {
                    final int[] array = newKey;
                    final int n2 = m;
                    array[n2] |= DES.bigbyte[k];
                }
                if (pcr[DES.pc2[k + 24]]) {
                    final int[] array2 = newKey;
                    final int n3 = n;
                    array2[n3] |= DES.bigbyte[k];
                }
            }
        }
        for (int i = 0; i != 32; i += 2) {
            final int i2 = newKey[i];
            final int i3 = newKey[i + 1];
            newKey[i] = ((i2 & 0xFC0000) << 6 | (i2 & 0xFC0) << 10 | (i3 & 0xFC0000) >>> 10 | (i3 & 0xFC0) >>> 6);
            newKey[i + 1] = ((i2 & 0x3F000) << 12 | (i2 & 0x3F) << 16 | (i3 & 0x3F000) >>> 4 | (i3 & 0x3F));
        }
        return newKey;
    }
    
    protected void desFunc(final int[] wKey, final byte[] in, final int inOff, final byte[] out, final int outOff) {
