...
public class XmlStreamReader extends Reader
{
    ...
    public XmlStreamReader(final InputStream is, final boolean lenient, final String defaultEncoding) throws IOException {
        ...
        this.encoding = this.doRawStream(is, lenient);
    ...
    public XmlStreamReader(final URLConnection conn, final String defaultEncoding) throws IOException {
        ...
        if (conn instanceof HttpURLConnection || contentType != null) {
            this.encoding = this.doHttpStream(is, contentType, lenient);
        }
        else {
            this.encoding = this.doRawStream(is, lenient);
        }
    ...
    public XmlStreamReader(final InputStream is, final String httpContentType, final boolean lenient, final String defaultEncoding) throws IOException {
        this.defaultEncoding = defaultEncoding;
        this.encoding = this.doHttpStream(is, httpContentType, lenient);
        this.reader = new InputStreamReader(is, this.encoding);
    }
    ...
    private String doRawStream(final InputStream is, final boolean lenient) throws IOException {
        ...
        try {
            ...
            if (lenient) {
                return this.doLenientDetection(null, is, ex);
            }
            throw ex;
        }
    }
    
    private String doHttpStream(final InputStream is, final String httpContentType, final boolean lenient) throws IOException {
        final BOMInputStream bom = new BOMInputStream(new BufferedInputStream(is, 4096), false, XmlStreamReader.BOMS);
        final BOMInputStream pis = new BOMInputStream(bom, true, XmlStreamReader.XML_GUESS_BYTES);
        final String bomEnc = bom.getBOMCharsetName();
        final String xmlGuessEnc = pis.getBOMCharsetName();
        ...
        try {
            ...
            if (lenient) {
                return this.doLenientDetection(httpContentType, is, ex);
            }
            throw ex;
        }
    }
    
    private String doLenientDetection(String httpContentType, final InputStream is, XmlStreamReaderException ex) throws IOException {
    ...
    static {
        ...
        XML_GUESS_BYTES = new ByteOrderMark[] { new ByteOrderMark("UTF-8", new int[] { 60, 63, 120, 109 }), new ByteOrderMark("UTF-16BE", new int[] { 0, 60, 0, 63 }), new ByteOrderMark("UTF-16LE", new int[] { 60, 0, 63, 0 }), new ByteOrderMark("CP1047", new int[] { 76, 111, 167, 148 }) };
