...
class MethodWriter extends MethodVisitor
{
        ...
        if (this.compute == 1) {
            ...
            for (int i = 0; i < nLocal; ++i) {
                if (local[i] instanceof String) {
                    final String desc = Type.getObjectType((String)local[i]).getDescriptor();
                    this.frame[frameIndex++] = Frame.type(this.cw, desc);
                }
                else if (local[i] instanceof Integer) {
                    this.frame[frameIndex++] = (0x1000000 | (int)local[i]);
                }
                else {
            ...
            for (int i = 0; i < nStack; ++i) {
                if (stack[i] instanceof String) {
                    final String desc = Type.getObjectType((String)stack[i]).getDescriptor();
                    this.frame[frameIndex++] = Frame.type(this.cw, desc);
                }
                else if (stack[i] instanceof Integer) {
                    this.frame[frameIndex++] = (0x1000000 | (int)stack[i]);
                }
                else {
    ...
    public void visitTypeInsn(final int opcode, final String type) {
        this.lastCodeOffset = this.code.length;
        final Item i = this.cw.newStringishItem(7, type);
        if (this.currentBlock != null) {
            if (this.compute == 0 || this.compute == 1) {
    ...
    public void visitMultiANewArrayInsn(final String desc, final int dims) {
        this.lastCodeOffset = this.code.length;
        final Item i = this.cw.newStringishItem(7, desc);
        if (this.currentBlock != null) {
            if (this.compute == 0 || this.compute == 1) {
    ...
    private void visitImplicitFirstFrame() {
        ...
        if ((this.access & 0x8) == 0x0) {
            if ((this.access & 0x80000) == 0x0) {
                ...
                this.frame[frameIndex++] = 16777222;
            }
        }
        int i = 1;
        while (true) {
            ...
            switch (this.descriptor.charAt(i++)) {
                ...
                case 'S':
                case 'Z': {
                    this.frame[frameIndex++] = 16777217;
                    continue;
                }
                case 'F': {
                    this.frame[frameIndex++] = 16777218;
                    continue;
                }
                case 'J': {
                    this.frame[frameIndex++] = 16777220;
                    continue;
                }
                case 'D': {
                    this.frame[frameIndex++] = 16777219;
                    continue;
                }
                case '[': {
