...
public class ClassReader
{
    ...
    private void readModule(final ClassVisitor classVisitor, final Context context, int u, final String mainClass, int packages) {
        ...
        final String name = this.readModule(u, buffer);
        final int flags = this.readUnsignedShort(u + 2);
        final String version = this.readUTF8(u + 4, buffer);
        u += 6;
        final ModuleVisitor mv = classVisitor.visitModule(name, flags, version);
        if (mv == null) {
            return;
        }
        if (mainClass != null) {
        ...
        if (packages != 0) {
            for (int i = this.readUnsignedShort(packages - 2); i > 0; --i) {
                final String packaze = this.readPackage(packages, buffer);
        ...
        for (int i = this.readUnsignedShort(u - 2); i > 0; --i) {
            final String module = this.readModule(u, buffer);
            final int access = this.readUnsignedShort(u + 2);
            final String requireVersion = this.readUTF8(u + 4, buffer);
            mv.visitRequire(module, access, requireVersion);
            u += 6;
        }
        u += 2;
        for (int i = this.readUnsignedShort(u - 2); i > 0; --i) {
            final String export = this.readPackage(u, buffer);
            final int access = this.readUnsignedShort(u + 2);
            final int exportToCount = this.readUnsignedShort(u + 4);
            u += 6;
            String[] tos = null;
            if (exportToCount != 0) {
        ...
        u += 2;
        for (int i = this.readUnsignedShort(u - 2); i > 0; --i) {
            final String open = this.readPackage(u, buffer);
            final int access = this.readUnsignedShort(u + 2);
            final int openToCount = this.readUnsignedShort(u + 4);
            u += 6;
            String[] tos = null;
            if (openToCount != 0) {
        ...
        u += 2;
        for (int i = this.readUnsignedShort(u - 2); i > 0; --i) {
            final String service = this.readClass(u, buffer);
            final int provideWithCount = this.readUnsignedShort(u + 2);
            u += 4;
            final String[] withs = new String[provideWithCount];
            for (int k = 0; k < withs.length; ++k) {
                withs[k] = this.readClass(u, buffer);
                u += 2;
            }
    ...
    private void readCode(final MethodVisitor mv, final Context context, int u) {
        ...
        for (int j = this.readUnsignedShort(u); j > 0; --j) {
            final Label start = this.createLabel(this.readUnsignedShort(u + 2), labels);
            final Label end = this.createLabel(this.readUnsignedShort(u + 4), labels);
            final Label handler = this.createLabel(this.readUnsignedShort(u + 6), labels);
            ...
            for (int k = stackMap; k < stackMap + stackMapSize - 2; ++k) {
                if (b[k] == 8) {
                    ...
                    if (v2 >= 0 && v2 < codeLength && (b[codeStart + v2] & 0xFF) == 0xBB) {
                        this.createLabel(v2, labels);
                    }
                }
            }
        }
        if ((context.flags & 0x100) != 0x0 && (context.flags & 0x8) != 0x0) {
            ...
            switch (ClassWriter.TYPE[opcode2]) {
                ...
                case 18: {
                    ...
                    if (opcode2 == 167 || opcode2 == 168) {
                        ...
                        final Label endif = this.createLabel(offset2 + 3, labels);
                ...
                case 19: {
                    mv.visitJumpInsn(200, labels[offset2 + this.readInt(u + 1)]);
                    insertFrame = true;
                    u += 5;
                    break;
                }
        ...
        if (tanns != null) {
            for (int i5 = 0; i5 < tanns.length; ++i5) {
                if (this.readByte(tanns[i5]) >> 1 == 32) {
                    int v = this.readAnnotationTarget(context, tanns[i5]);
        ...
        if (itanns != null) {
            for (int i5 = 0; i5 < itanns.length; ++i5) {
                if (this.readByte(itanns[i5]) >> 1 == 32) {
                    int v = this.readAnnotationTarget(context, itanns[i5]);
    ...
    private int readFrame(int stackMap, final boolean zip, final boolean unzip, final Context frame) {
        ...
        this.createLabel(frame.offset += delta + 1, labels);
        return stackMap;
    }
    ...
    private Label createLabel(final int offset, final Label[] labels) {
        final Label label2;
        final Label label = label2 = this.readLabel(offset, labels);
    ...
    private void createDebugLabel(final int offset, final Label[] labels) {
        if (labels[offset] == null) {
            final Label label = this.readLabel(offset, labels);
            label.status |= 0x1;
        }
    }
