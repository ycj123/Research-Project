...
public class SvnXpp3Reader
{
    ...
    private Settings parseSettings(final XmlPullParser parser, final boolean strict) throws IOException, XmlPullParserException {
        ...
        while ((strict ? parser.nextTag() : this.nextTag(parser)) == 2) {
            if (this.checkFieldWithDuplicate(parser, "configDirectory", null, parsed)) {
                settings.setConfigDirectory(this.getTrimmedValue(parser.nextText()));
            }
            else if (this.checkFieldWithDuplicate(parser, "useCygwinPath", null, parsed)) {
                settings.setUseCygwinPath(this.getBooleanValue(this.getTrimmedValue(parser.nextText()), "useCygwinPath", parser, "false"));
            }
            else if (this.checkFieldWithDuplicate(parser, "cygwinMountPath", null, parsed)) {
                settings.setCygwinMountPath(this.getTrimmedValue(parser.nextText()));
            }
            else if (this.checkFieldWithDuplicate(parser, "useNonInteractive", null, parsed)) {
                settings.setUseNonInteractive(this.getBooleanValue(this.getTrimmedValue(parser.nextText()), "useNonInteractive", parser, "true"));
            }
            else if (this.checkFieldWithDuplicate(parser, "useAuthCache", null, parsed)) {
                settings.setUseAuthCache(this.getBooleanValue(this.getTrimmedValue(parser.nextText()), "useAuthCache", parser, "false"));
            }
            else if (this.checkFieldWithDuplicate(parser, "trustServerCert", null, parsed)) {
                settings.setTrustServerCert(this.getBooleanValue(this.getTrimmedValue(parser.nextText()), "trustServerCert", parser, "false"));
            }
            else {
    ...
    private Settings read(final XmlPullParser parser, final boolean strict) throws IOException, XmlPullParserException {
        ...
        while (eventType != 1) {
            if (eventType == 2) {
                if (strict && !"svn-settings".equals(parser.getName())) {
                    throw new XmlPullParserException("Expected root element 'svn-settings' but found '" + parser.getName() + "'", parser, null);
                }
        ...
        throw new XmlPullParserException("Expected root element 'svn-settings' but found no element at all: invalid XML document", parser, null);
    }
