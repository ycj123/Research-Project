...
public class GitXpp3Reader
{
    ...
    private Settings parseSettings(final XmlPullParser parser, final boolean strict) throws IOException, XmlPullParserException {
        ...
        while ((strict ? parser.nextTag() : this.nextTag(parser)) == 2) {
            if (this.checkFieldWithDuplicate(parser, "revParseDateFormat", null, parsed)) {
                settings.setRevParseDateFormat(this.getTrimmedValue(parser.nextText()));
            }
            else if (this.checkFieldWithDuplicate(parser, "traceGitCommand", null, parsed)) {
                settings.setTraceGitCommand(this.getTrimmedValue(parser.nextText()));
            }
            else if (this.checkFieldWithDuplicate(parser, "commitNoVerify", null, parsed)) {
                settings.setCommitNoVerify(this.getBooleanValue(this.getTrimmedValue(parser.nextText()), "commitNoVerify", parser, "false"));
            }
            else {
    ...
    private Settings read(final XmlPullParser parser, final boolean strict) throws IOException, XmlPullParserException {
        ...
        while (eventType != 1) {
            if (eventType == 2) {
                if (strict && !"git-settings".equals(parser.getName())) {
                    throw new XmlPullParserException("Expected root element 'git-settings' but found '" + parser.getName() + "'", parser, null);
                }
        ...
        throw new XmlPullParserException("Expected root element 'git-settings' but found no element at all: invalid XML document", parser, null);
    }
