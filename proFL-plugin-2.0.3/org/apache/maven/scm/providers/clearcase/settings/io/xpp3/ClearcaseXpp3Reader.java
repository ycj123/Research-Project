...
public class ClearcaseXpp3Reader
{
    ...
    private Settings parseSettings(final XmlPullParser parser, final boolean strict) throws IOException, XmlPullParserException {
        ...
        while ((strict ? parser.nextTag() : this.nextTag(parser)) == 2) {
            if (this.checkFieldWithDuplicate(parser, "viewstore", null, parsed)) {
                settings.setViewstore(this.getTrimmedValue(parser.nextText()));
            }
            else if (this.checkFieldWithDuplicate(parser, "useVWSParameter", null, parsed)) {
                settings.setUseVWSParameter(this.getBooleanValue(this.getTrimmedValue(parser.nextText()), "useVWSParameter", parser, "true"));
            }
            else if (this.checkFieldWithDuplicate(parser, "clearcaseType", null, parsed)) {
                settings.setClearcaseType(this.getTrimmedValue(parser.nextText()));
            }
            else if (this.checkFieldWithDuplicate(parser, "changelogUserFormat", null, parsed)) {
                settings.setChangelogUserFormat(this.getTrimmedValue(parser.nextText()));
            }
            else {
    ...
    private Settings read(final XmlPullParser parser, final boolean strict) throws IOException, XmlPullParserException {
        ...
        while (eventType != 1) {
            if (eventType == 2) {
                if (strict && !"clearcase-settings".equals(parser.getName())) {
                    throw new XmlPullParserException("Expected root element 'clearcase-settings' but found '" + parser.getName() + "'", parser, null);
                }
        ...
        throw new XmlPullParserException("Expected root element 'clearcase-settings' but found no element at all: invalid XML document", parser, null);
    }
