...
public class MavenToolchainsXpp3Reader
{
    ...
    private PersistedToolchains parsePersistedToolchains(final String tagName, final XmlPullParser parser, final boolean strict) throws IOException, XmlPullParserException {
        final PersistedToolchains persistedToolchains = new PersistedToolchains();
        ...
        while (eventType != 1) {
            if (eventType == 2) {
                if (parser.getName().equals(tagName)) {
                    ...
                    if (parser.getName().equals("toolchain")) {
                        List toolchains = persistedToolchains.getToolchains();
                        if (toolchains == null) {
                            toolchains = new ArrayList();
                            persistedToolchains.setToolchains(toolchains);
                        }
                        toolchains.add(this.parseToolchainModel("toolchain", parser, strict));
                    }
                    else if (strict) {
    ...
    private ToolchainModel parseToolchainModel(final String tagName, final XmlPullParser parser, final boolean strict) throws IOException, XmlPullParserException {
        final ToolchainModel toolchainModel = new ToolchainModel();
        final Set parsed = new HashSet();
        while (parser.nextTag() == 2) {
            if (this.checkFieldWithDuplicate(parser, "type", null, parsed)) {
                toolchainModel.setType(this.getTrimmedValue(parser.nextText()));
            }
            else if (this.checkFieldWithDuplicate(parser, "provides", null, parsed)) {
    ...
    public PersistedToolchains read(final Reader reader, final boolean strict) throws IOException, XmlPullParserException {
        ...
        parser.next();
        return this.parsePersistedToolchains("toolchains", parser, strict);
    }
    
    public PersistedToolchains read(final Reader reader) throws IOException, XmlPullParserException {
        return this.read(reader, true);
    }
    
    public PersistedToolchains read(final InputStream in, final boolean strict) throws IOException, XmlPullParserException {
        final Reader reader = (Reader)ReaderFactory.newXmlReader(in);
        return this.read(reader, strict);
    }
    
    public PersistedToolchains read(final InputStream in) throws IOException, XmlPullParserException {
        final Reader reader = (Reader)ReaderFactory.newXmlReader(in);
