...
public class ASTLTNode extends SimpleNode
{
    ...
    public ASTLTNode(final Parser p, final int id) {
        super(p, id);
    }
    ...
    public boolean evaluate(final InternalContextAdapter context) throws MethodInvocationException {
        ...
        if (!(left instanceof Number) || !(right instanceof Number)) {
            this.log.error(((left instanceof Number) ? "Right" : "Left") + " side of '>=' operation is not a valid Number. " + context.getCurrentTemplateName() + " [line " + this.getLine() + ", column " + this.getColumn() + "]");
            return false;
        }
