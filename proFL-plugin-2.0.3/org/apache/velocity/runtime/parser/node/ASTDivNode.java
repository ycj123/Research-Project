...
public class ASTDivNode extends SimpleNode
{
    ...
    public ASTDivNode(final Parser p, final int id) {
        super(p, id);
    }
    ...
    public Object value(final InternalContextAdapter context) throws MethodInvocationException {
        ...
        if (left == null || right == null) {
            this.log.error(((left == null) ? "Left" : "Right") + " side (" + this.jjtGetChild((left != null) ? 1 : 0).literal() + ") of division operation has null value." + " Operation not possible. " + context.getCurrentTemplateName() + " [line " + this.getLine() + ", column " + this.getColumn() + "]");
            return null;
        }
        ...
        if (!(left instanceof Number) || !(right instanceof Number)) {
            this.log.error(((left instanceof Number) ? "Right" : "Left") + " side of division operation is not a number. " + context.getCurrentTemplateName() + " [line " + this.getLine() + ", column " + this.getColumn() + "]");
            return null;
        }
