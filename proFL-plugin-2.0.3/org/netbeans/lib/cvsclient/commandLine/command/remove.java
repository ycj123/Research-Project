...
public class remove extends AbstractCommandProvider
{
    ...
    public Command createCommand(final String[] array, final int n, final GlobalOptions globalOptions, String property) {
        final RemoveCommand removeCommand = new RemoveCommand();
        removeCommand.setBuilder(null);
        final GetOpt getOpt = new GetOpt(array, removeCommand.getOptString());
        getOpt.optIndexSet(n);
        boolean b = false;
        int getopt;
        while ((getopt = getOpt.getopt()) != -1) {
            if (!removeCommand.setCVSCommand((char)getopt, getOpt.optArgGet())) {
                b = true;
            }
        }
        if (b) {
